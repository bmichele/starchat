...
import com.getjenny.analyzer.atoms.{CheckVariableContainsValue, CheckVariableValue}
...
class AtomsTest extends AnyWordSpec with Matchers with ScalatestRouteTest {
  ...
  "checkVariableContainsValue" should {
    "return score 1 when variable contains substring" in {
      ...
      k.evaluate(query, AnalyzersDataInternal(extractedVariables = analyzerData)).score should be(1.0)
    }
    ...
    "return score 0 when variable does not contain substring" in {
      ...
      k.evaluate(query, AnalyzersDataInternal(extractedVariables = analyzerData)).score should be(0.0)
    }
