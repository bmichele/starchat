...
import com.getjenny.starchat.entities.io.{Permissions, ReturnMessageData, SynExtractionRequest, TermsExtractionRequest}
...
trait TermsExtractionResource extends StarChatResource {
  ...
  def termsExtractionRoutes: Route = handleExceptions(routesExceptionHandler) {
    pathPrefix(indexRegex ~ Slash ~ """extraction""" ~ Slash ~ """keywords""") { indexName =>
      post {
  ...
  def freqExtractionRoutes: Route = handleExceptions(routesExceptionHandler) {
    pathPrefix(indexRegex ~ Slash ~ """extraction""" ~ Slash ~ """frequencies""") { indexName =>
      post {
        authenticateBasicAsync(realm = authRealm,
          authenticator = authenticator.authenticator) { user =>
          authorizeAsync(_ =>
  ...
  def synExtractionRoutes: Route = handleExceptions(routesExceptionHandler) {
    pathPrefix(indexRegex ~ Slash ~ """extraction""" ~ Slash ~ """synonyms""") { indexName =>
      post {
        authenticateBasicAsync(realm = authRealm,
          authenticator = authenticator.authenticator) { user =>
          authorizeAsync(_ =>
