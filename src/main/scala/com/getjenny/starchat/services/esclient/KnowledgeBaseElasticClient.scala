...
object KnowledgeBaseElasticClient extends QuestionAnswerElasticClient {
  ...
  override val indexSuffix: String = config.getString("es.kb_index_suffix")
  ...
  override val numberOfShards: Int = config.getInt("es.kb_idx_number_of_shards")
  override val numberOfReplicas: Int = config.getInt("es.kb_idx_number_of_replicas")
}
