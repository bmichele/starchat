...
trait SystemElasticClient extends ElasticClient {
  ...
  override val numberOfShards: Int = config.getInt("es.system_idx_number_of_shards")
  override val numberOfReplicas: Int = config.getInt("es.system_idx_number_of_replicas")
